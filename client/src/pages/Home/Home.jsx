...
const HomePage = ({ blogData }) => {
  ...
  // Safe Window Width Check (SSR hydration issue fix)
  useEffect(() => {
    setMounted(true);
  ...
  return (
    <div className="mb-10 mt-7 px-3 md:px-8">
      {/* Dynamic Keyframe Injection (Clean & Safe without styled-jsx) */}
      <style>{`
        @keyframes paperFlip {
          0%, 100% {
  ...
  <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-12">
    ...
    <div className="flex flex-col items-start gap-5 max-w-2xl text-left w-full">
      ...
      {/* SEO H1 + Mobile Book Icon (Tight gap on mobile) */}
      <div className="flex items-center justify-between gap-1 sm:gap-2 w-full">
        ...
        {/* Mobile-Only Book Icon (Compact, Scaled & Shifted closer) */}
  ...
  {[
    ...
    <Link 
      key={tag.name}
      href={`${process.env.NEXT_PUBLIC_SITE_URL}/blogs/${tag.slug}`}
      className="text-[11px] font-medium px-2.5 py-1 rounded-md bg-muted/40 hover:bg-red-500/10 hover:text-red-600 border border-border/40 transition-colors inline-block"
    >
      {tag.name}
    </Link>
  ))}
      ...
      {/* Live Metrics Bar */}
      <div className="flex flex-wrap items-center gap-6 pt-2 text-xs text-muted-foreground border-t border-border/40 w-full max-w-lg">
        
        {/* Animated Refresh / Update Icon */}
        <div className="flex items-center gap-2 text-red-600 dark:text-red-400 font-medium">
          <svg 
            className="w-3.5 h-3.5 animate-spin text-red-500" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
        ...
        {/* Dynamic Incrementing Readers Counter */}
        ...
        <div>
          <strong className="font-semibold text-foreground">50+</strong> Articles
        </div>
    ...
    {/* Desktop-Only Dynamic Book / Page Flipper */}
  ...
  {/* Soft Gradient Line blending into feed */}
  <div className="mt-8 md:mt-12 h-px w-full bg-gradient-to-r from-transparent via-border to-transparent" />
      ...
      {blogs.length > 0 && visibleCount < blogs.length && (
        <div className="flex justify-center mt-8">
          <Button 
            onClick={() => setVisibleCount((prev) => prev + (typeof window !== "undefined" && window.innerWidth >= 768 ? 50 : 10))} 
            variant="outline"
          >
            See More
          </Button>
